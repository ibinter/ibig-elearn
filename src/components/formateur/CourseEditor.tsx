...
import {
  ...
  ArrowLeft, Loader2, Globe, HelpCircle, X
} from 'lucide-react'
...
// ─── Quiz Editor (inline dans le CourseEditor) ───────────────────────────────
...
function QuizEditor({ lessonId }: { lessonId: string }) {
  ...
  const newQ = (): QuizQuestion => ({
    ...
    explanation: '', position: questions.length,
  })
  ...
  const saveQuestion = async (idx: number) => {
    ...
    try {
      if (q.id) {
        await fetch('/api/formateur/quiz', {
          method: 'PATCH', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ id: q.id, question: q.question, options: q.options, correct_option: q.correct_option, explanation: q.explanation }),
        })
      } else {
        const res = await fetch('/api/formateur/quiz', {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ lesson_id: lessonId, question: q.question, options: q.options, correct_option: q.correct_option, explanation: q.explanation, position: q.position }),
        })
        const data = await res.json()
        if (res.ok) updateQ(idx, { id: data.id })
      }
  ...
  if (loading) return <div className="text-xs text-gray-400 py-2">Chargement du quiz…</div>

  return (
    <div className="space-y-2 border border-blue-100 bg-blue-50/30 rounded-xl p-3">
      <div className="flex items-center justify-between">
        <span className="text-xs font-semibold text-[#0B3D91] flex items-center gap-1.5">
          <HelpCircle className="w-3.5 h-3.5" /> Questions ({questions.length})
        </span>
        <button onClick={addQuestion}
          className="text-xs bg-[#0B3D91] text-white px-2.5 py-1 rounded-lg hover:bg-blue-800 flex items-center gap-1">
          <Plus className="w-3 h-3" /> Ajouter
        </button>
      </div>

      {questions.map((q, qIdx) => (
        <div key={qIdx} className="bg-white rounded-lg border border-gray-100 overflow-hidden">
          {/* Header question */}
          <div className="flex items-center gap-2 px-3 py-2">
            <span className="text-xs font-bold text-gray-400 w-5 flex-shrink-0">Q{qIdx + 1}</span>
            <button onClick={() => setExpanded(expanded === qIdx ? null : qIdx)}
            ...
            <div className="flex items-center gap-1 flex-shrink-0">
              <button onClick={() => saveQuestion(qIdx)} disabled={saving === qIdx}
                className="text-xs text-green-700 bg-green-50 hover:bg-green-100 px-2 py-1 rounded-md flex items-center gap-0.5">
                {saving === qIdx ? <Loader2 className="w-3 h-3 animate-spin" /> : <Save className="w-3 h-3" />}
              </button>
              <button onClick={() => deleteQuestion(qIdx)} className="text-gray-300 hover:text-red-500">
                <Trash2 className="w-3.5 h-3.5" />
              </button>
              <button onClick={() => setExpanded(expanded === qIdx ? null : qIdx)} className="text-gray-300">
                {expanded === qIdx ? <ChevronDown className="w-3.5 h-3.5" /> : <ChevronRight className="w-3.5 h-3.5" />}
              </button>
            </div>
          </div>
          ...
          {expanded === qIdx && (
            <div className="border-t border-gray-50 px-3 py-3 space-y-3">
              <textarea value={q.question}
                ...
                className="w-full text-xs px-3 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-[#0B3D91] resize-none"
              />

              <div className="space-y-1.5">
                <p className="text-xs font-medium text-gray-500">Options (sélectionner la bonne réponse)</p>
                {q.options.map((opt, oIdx) => (
                  <div key={oIdx} className="flex items-center gap-2">
                    <button onClick={() => updateQ(qIdx, { correct_option: oIdx })}
                      className={`w-5 h-5 rounded-full border-2 flex-shrink-0 flex items-center justify-center text-xs font-bold transition-colors ${
                        q.correct_option === oIdx ? 'bg-green-500 border-green-500 text-white' : 'border-gray-300 text-gray-400 hover:border-green-400'
                      }`}>
                      {String.fromCharCode(65 + oIdx)}
                    </button>
                    <input value={opt} onChange={e => updateOption(qIdx, oIdx, e.target.value)}
                      placeholder={`Option ${String.fromCharCode(65 + oIdx)}`}
                      className="flex-1 text-xs px-2.5 py-1.5 border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-[#0B3D91]"
                    />
                  </div>
                ))}
              </div>
              ...
              <input value={q.explanation}
                ...
                className="w-full text-xs px-3 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-[#0B3D91]"
              />
            </div>
          )}
        </div>
      ...
      {questions.length === 0 && (
        <p className="text-xs text-gray-400 text-center py-2">Aucune question — cliquez sur Ajouter.</p>
      )}
    </div>
  )
}
                  ...
                  {mod.lessons.map((les, lIdx) => {
                    ...
                    return (
                      <div key={lIdx} className="px-4 py-3 space-y-3">
                        ...
                        {les.type === 'quiz' && les.id && (
                          <div className="ml-7">
                            <QuizEditor lessonId={les.id} />
                          </div>
                        )}
                        {les.type === 'quiz' && !les.id && (
                          <p className="ml-7 text-xs text-amber-600 bg-amber-50 px-3 py-2 rounded-lg">
                            💾 Sauvegardez d&apos;abord cette leçon pour ajouter des questions.
                          </p>
                        )}
                      </div>
                    )
